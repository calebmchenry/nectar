...
- **A1 (Manager loop handler) + A2 (loop_restart) + A3 (Tool call hooks)** — the remaining attractor engine execution-control gaps. The Codex draft provides an excellent architectural blueprint for this work: explicit telemetry namespace (`stack.child.*`), filesystem control plane with `manager-steer.json`, atomic consumption semantics, restart manifest linkage, and tool hook contracts. L6 (OpenAI-compatible adapter) can pair with Sprint 017 or stand alone as Sprint 018.
