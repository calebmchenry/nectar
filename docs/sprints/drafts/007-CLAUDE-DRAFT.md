...
**Goal:** Make `box` / `codergen` nodes autonomous coding agents. After this sprint, a codergen node receives a prompt, reads files, edits code, runs commands, iterates through tool calls, and returns a final answer — all within bounded turn limits and with full artifact preservation.
...
└── tools/
    ...
    ├── grep.ts                 # In-process regex search
    └── glob.ts                 # In-process workspace file matching
```
...
**Tasks:**
- [ ] Define `SessionConfig`: max_turns (default 12), max_tool_rounds_per_input (default 8), default_command_timeout_ms (default 120_000), workspace_root
...
- [ ] Implement `LocalExecutionEnvironment`:
  - Constructor takes `workspace_root: string`
...
- [ ] `read_file(path, offset?, limit?)`:
  ...
  - Error if file doesn't exist or is binary (detect via null bytes in first 8KB)
...
- [ ] `edit_file(path, old_string, new_string)`:
  ...
  - Find `old_string` in content — if zero matches, return error with helpful message ("old_string not found in file — did the content change? Try read_file first")
  - If multiple matches, return error with match count and line numbers ("found 3 matches for old_string at lines 12, 45, 89 — provide more context to make the match unique")
...
- [ ] `grep(pattern, path?, include?, max_results?)`:
  ...
  - Respect `.gitignore` patterns (use existing ignore logic or simple top-level .gitignore parsing)
...
- [ ] Implement `AnthropicProfile`:
  - System prompt: role framing ("You are an expert coding assistant"), capabilities list, tool usage guidelines emphasizing exact-match edits, response format expectations
...
- [ ] Implement project instruction discovery:
  - Walk from workspace root upward (stop at root)
  ...
  - Apply 32KB budget: if total exceeds budget, truncate least-specific files first (AGENTS.md gets cut before provider-specific file)
...
- [ ] Engine passes `process.cwd()` as workspace root and an event bridge into handler input
...
- [ ] Update CLI renderer for agent events:
  ...
  - `agent_loop_detected`: `  ⚠️  Loop detected — aborting session`
...
- [ ] Add `ajv` to `package.json` dependencies for tool argument validation
...
| File | Action | Purpose |
|------|--------|---------|
| `package.json` | Modify | Add `ajv` for JSON Schema validation |
...
| `src/agent-loop/tools/read-file.ts` | Create | File reading with line numbers and slicing |
...
| `src/agent-loop/tools/grep.ts` | Create | In-process regex search |
| `src/agent-loop/tools/glob.ts` | Create | In-process file matching |
...
- [ ] `edit_file` performs exact-match replacement on unique matches
...
| Scope creeps into "full Claude Code parity" | High | High | Explicit, opinionated scope cuts: no steer/follow_up, no subagents, no remote environments, no provider-specific edit DSLs, no parallel tool execution. Ship the smallest complete local agent loop. |
| edit_file is too fragile for real use | Medium | Medium | Return helpful error messages with line numbers. Models can recover by re-reading the file. This is the same contract Claude Code uses. |
| Shell commands damage the repo | Medium | High | Same threat model as Makefile — user explicitly runs `pollinator run`. Workspace sandboxing prevents path escapes. Env filtering prevents secret leakage. Document the security model. |
...
| ajv dependency adds bloat | Low | Low | ajv is mature, well-maintained, and the standard for JSON Schema validation in Node. The alternative (hand-rolled validation) is more error-prone and harder to maintain. |
