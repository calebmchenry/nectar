...
The alternative sprint candidate (A4/A5 context fidelity) improves execution quality in long pipelines — a subtler win. This sprint unblocks an entire product surface (Swarm Intelligence) and delivers measurable cost savings on day one. Fidelity should follow.
...
- L7 (Middleware/interceptor chain) — orthogonal architecture, separate sprint
- L8 (Model catalog / `ModelInfo`) — informational convenience, not blocking
- L9 (High-level `generate()` with tool loop) — the agent-loop already handles tool loops; SDK convenience deferred
...
- A4/A5 (Context fidelity runtime) — attractor engine work, next sprint
- Swarm analysis implementation itself — this sprint provides the SDK foundation; seedbed sprint builds on it

**Cut-line:** If the sprint runs long, defer Phase 4 (`stream_object()` streaming structured output). Ship `generate_object()` for non-streaming structured output, which covers the swarm analysis use case completely. Do **not** defer L2 (signature) or L10 (caching) — these are correctness and cost issues that affect every Anthropic call today.
...
**`stream_object()` flow (Phase 4, deferrable):**
...
**Schema validation uses `ajv`** — already in `package.json` via agent-loop's `ToolRegistry`. No new dependency. Import directly rather than reinventing JSON Schema validation.
...
- A4/A5 (Context fidelity runtime + thread resolution) — highest-impact attractor engine gaps, now that the LLM client foundation is production-grade
