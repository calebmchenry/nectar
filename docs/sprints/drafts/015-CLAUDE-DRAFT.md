...
- **A4 and A5 are the last medium-severity attractor engine gaps.** Closing them makes the engine the first spec to reach zero medium+ issues. Every other engine gap is Low.
...
- **A11 and C3 are trivial add-ons that close five more Low gaps for free.** `auto_status` is a 10-line engine change. `reasoning_effort` mid-session override is required for `full` fidelity to work when two nodes on the same thread want different reasoning levels.
...
- Thread ID resolution: node → edge → subgraph class → previous node
...
- A1 (Manager loop handler) — separate handler type, next sprint candidate
- A2 (HTTP server mode) — product surface, not engine internals
...
1. **Stateful plan → implement → review chain.** Three `box` nodes share `thread_id="feature"` with `fidelity="full"`. `plan` creates a live `AgentSession`, `implement` follows up on that same conversation via `session.followUp()`, and `review` follows up again. The LLM remembers everything discussed — no re-prompting, no synthetic summaries, no token waste on redundant context.

2. **Bounded context for routing decisions.** A "triage" node with `fidelity="compact"` receives a structured Markdown table of completed nodes — ID, outcome, duration, key context updates. No raw conversation history. The LLM assesses pipeline state in ~1000 tokens instead of 50K.
...
8. **Inspectable run state.** After any run, `.nectar/cocoons/<run-id>/manifest.json` tells you what ran, `checkpoint.json` tells you where it stopped, and `artifacts/index.json` shows what was produced. No scraping handler-specific logs.
...
**`RunStore`** owns this directory and exposes `initialize()`, `writeCheckpoint()`, `readCheckpoint()`, `readManifest()`, `listRuns()`, and `artifactStore()`. For one sprint, it dual-writes the legacy flat `<run-id>.json` for backward compatibility.
...
The fallback is `compact`, not `full`. This is opinionated: unbounded context is the wrong default for production pipelines. Pipelines that want `full` should say so. Existing pipelines without any fidelity attributes get compact summaries — a minor behavioral change that produces better results.

### Thread ID Resolution

Per spec §5.4, resolved with this chain (first non-null wins):

1. Target node `thread_id` attribute
2. Incoming edge `thread_id` attribute
3. First class in `node.classes` (subgraph class derivation)
4. Previous completed node's thread ID (continuity default)
...
|------|---------|----------|--------|
| `full` | Reuse live `AgentSession` via `SessionRegistry` | Graph goal only | No cap (model context window is the limit) |
| `truncate` | Fresh | Graph goal + tail of prior thread messages | 400 chars |
...
All non-`full` modes use deterministic templates (no LLM call). Character budgets are hard caps enforced by truncation. Truncation prioritizes: header (goal, run_id) > most recent failure/retry > most recent human answer > recent successful nodes. Oldest successful node summaries drop first.
...
- Provider/model signature frozen at creation — mismatched reuse fails fast
- `reasoning_effort` can vary per-turn (applied on next LLM call)
- Per-thread FIFO lock: parallel branches sharing a thread key serialize, never interleave
...
resolve thread_key (node → edge → class → previous)
...
handler outcome
   │
   ▼
store large payloads in ArtifactStore
write thread conversation back (if full fidelity)
   │
   ▼
...
- [ ] Filter context: exclude `internal.*`, raw stdout/stderr blobs, full response payloads
...
- [ ] Implement `resolveThreadId(node, incomingEdge, graph, previousThreadId): string | null`
  - Resolution: node attr → edge attr → first subgraph class → previous node thread
...
- [ ] Create `SessionRegistry` class:
  ...
  - Per-thread FIFO lock for concurrent access
...
- [ ] Add per-turn `reasoning_effort` override to `AgentSession`:
  ...
  - No session replacement needed
- [ ] Add `getConversation(): Message[]` to `AgentSession` (returns copy)
- [ ] Extend checkpoint with `resume_requires_degraded_fidelity: boolean`
...
- [ ] Tests:
  ...
  - Thread resolver: edge attr wins over subgraph class
...
- [ ] After codergen node completes with `full` fidelity: session stays alive in registry (not extracted)
- [ ] Non-codergen nodes (tool, conditional, etc.) ignore fidelity — unaffected
- [ ] Extend `Cocoon` with `thread_registry_keys?: string[]` — records which threads existed (sessions not serialized, but we track what was active for degraded-resume)
...
| `src/engine/thread-resolver.ts` | Create | `resolveThreadId()` — 4-step resolution chain |
...
| `src/cli/commands/resume.ts` | Modify | Prefer canonical checkpoints |
...
| `test/engine/thread-resolver.test.ts` | Create | Thread ID resolution chain |
| `test/engine/session-registry.test.ts` | Create | Session reuse, locking, mismatch, cleanup |
...
- [ ] Existing pipelines without fidelity/thread attributes get `compact` default (minor behavioral change from previous "pass everything")
...
- [ ] Every run writes `.nectar/cocoons/<run-id>/checkpoint.json` after each node
...
- [ ] `resolveThreadId()` implements resolution chain: node → edge → class → previous
...
- [ ] Thread resolution: all 4 chain steps
- [ ] SessionRegistry: reuse, mismatch, locking, cleanup
...
- [ ] ArtifactStore: inline, file-backed, CRUD
...
|------|------------|--------|------------|
| **Default fidelity change (`compact` instead of implicit `full`) breaks existing pipelines** | Medium | Medium | This is intentional — unbounded context is a bad default. Pipelines wanting `full` should declare it. Add a migration note in release changelog. Integration tests verify existing pipelines still work (they just get terser context). |
