# Sprint 024: Green Suite, Named Retry Presets, Streaming Reasoning Lifecycle & Error Taxonomy

## Overview

**Goal:** Fix the 3 persistent test failures that have been red since Sprint 022, then close the 4 highest-impact remaining compliance gaps: named retry preset policies (GAP-4), streaming reasoning event lifecycle (GAP-6), error subtypes for quota exhaustion and stream errors (GAP-3), and structured timeout configuration (GAP-8). After this sprint, the test suite is green, retry behavior is tunable by pipeline authors, streaming consumers get proper reasoning lifecycle events, error classification is precise, and timeout control is granular.
...
2. **GAP-4 (named retry presets) is the last engine-level compliance gap.** Every engine feature — parallel execution, fan-in, conditions, checkpointing, manager loops, composition — is implemented. But pipeline authors cannot select `standard`, `aggressive`, `linear`, or `patient` retry policies by name. The spec requires this. It directly affects how real pipelines handle transient failures, and it's the only remaining attractor-spec gap.

3. **GAP-6 (streaming reasoning lifecycle) breaks the spec's universal content pattern.** Text and tool calls follow `start → delta → end`. Reasoning/thinking emits only `delta` events with no lifecycle boundaries. Stream consumers cannot reliably detect when reasoning begins or ends, which breaks progressive UI rendering and resource management. This is a straightforward gap in the unified LLM layer.

4. **GAP-3 and GAP-8 are small but load-bearing.** Distinguishing `QuotaExceededError` from `RateLimitError` determines whether retry is appropriate (quota = no, rate limit = yes). `StreamError` as a distinct type lets consumers handle mid-stream disconnects differently from connection failures. `TimeoutConfig` gives adapters proper connect/request/stream_read granularity instead of one flat number. Together these close the error and timeout sections of the unified-llm-spec.

5. **After this sprint, only optional/future-looking gaps remain.** GAP-1 (AUDIO/DOCUMENT content types) is for modalities no provider fully offers today. GAP-2 (Gemini extended tools) is explicitly optional per spec. GAP-5 (edit_file fuzzy matching) is "may" in the spec. GAP-7 (incremental JSON parsing) is a UX optimization. None of these block real-world pipeline execution.
...
| GAP-3: QuotaExceeded, StreamError subtypes | unified-llm-spec §6.1 | Small | Correct retry/no-retry classification |
| GAP-8: Structured TimeoutConfig | unified-llm-spec §4.7 | Small | Granular timeout control for adapters |

**Deliberately deferred:**

- GAP-1: AUDIO/DOCUMENT content types — no provider fully uses these today
...
**Cut line:** If the sprint compresses, cut GAP-8 (TimeoutConfig) first, then GAP-3 (error subtypes). Do not cut the test fixes, GAP-4 (retry presets), or GAP-6 (reasoning lifecycle). Those are the load-bearing deliverables.

**Out of scope:**

- New CLI commands or Hive UI features
- New handler types
- LLM adapter changes beyond streaming lifecycle and error taxonomy
...
1. **Retry policy by name:** A pipeline author writes `max_retries=5; retry_policy="patient"` on an LLM node that calls a rate-limited API. The engine uses 2s initial delay with 3.0x factor instead of the default 200ms/2.0x. A different node uses `retry_policy="aggressive"` for fast recovery from transient network blips.

2. **Graph-level default retry policy:** A garden sets `default_retry_policy="standard"` at graph level. All nodes without explicit policies inherit 5-attempt, 200ms-base, 2.0x-factor backoff. Individual nodes can override.

3. **Reasoning lifecycle in the Hive:** A user runs a pipeline with a Claude node that uses extended thinking. The Hive UI shows a "Thinking..." indicator when `reasoning_start` fires, streams thinking tokens via `reasoning_delta`, and collapses the thinking panel on `reasoning_end`. Without lifecycle events, the UI cannot know when thinking begins or ends.
...
5. **Mid-stream disconnect recovery:** An SSE connection drops mid-response. The adapter raises `StreamError` with the partial content received so far. The retry middleware knows not to retry (content was already yielded) but the consumer can distinguish "stream broke" from "server never responded."

6. **Granular timeout control:** A long-running Gemini call needs 5 minutes for the full response but only 10 seconds to establish the connection. `TimeoutConfig` allows `connect: 10_000, request: 300_000` instead of a single 300s timeout that hides connection failures.

7. **Green CI gate:** A contributor opens a PR. CI runs `npm test` and all 967+ tests pass. The contributor does not need to investigate whether failures are "known" or new — the suite is authoritative.
...
2. **Retry presets are data, not code.** Named presets are a lookup table mapping name → config values. The existing retry infrastructure (`src/engine/retry.ts`) already accepts configurable parameters — presets are a thin convenience layer on top.

3. **Streaming lifecycle follows the existing pattern.** Text has `content_delta`. Tool calls have `tool_call_delta`. Reasoning gets `thinking_start`, `thinking_delta`, `thinking_end` — same naming convention, same event flow. No new abstractions.

4. **Error taxonomy extends, never replaces.** `QuotaExceededError` extends `LLMError` alongside the existing hierarchy. `StreamError` does the same. Existing catch blocks that match `RateLimitError` are unaffected.

5. **TimeoutConfig is additive.** The existing `timeout_ms` field continues to work as a shorthand for "total timeout." `TimeoutConfig` is an alternative that provides granularity. Adapters use `TimeoutConfig` internally; the public API accepts either.
...
**hive-run-flow timeout:** The full HTTP integration flow (preview → save → run → question → cancel → resume → replay) times out at 5s test + 10s hook. Root cause is likely the cancel/resume race that Sprint 023 partially fixed — the `pending_abort_reason` mechanism works but the broader flow still has a timing window. Fix: ensure the engine attachment and abort queueing happen within the test's timing budget, increase test timeout if the flow is genuinely slower.

**pipeline-events missing `pipeline_failed`:** The engine emits `stage_failed` when a node fails (engine.ts ~line 471) but `pipeline_failed` (emitted by `finishError()` at ~line 990) only fires when no next edge is found. When a node fails and there IS a failure edge, `pipeline_failed` is never emitted. Fix: emit `pipeline_failed` when the pipeline reaches a terminal failure state (exit node with failure status, or no remaining edges after a failure), not only on "no edge found."
...
- **Gemini adapter:** Emit `thinking_start` before the first `thinking_delta` in a thought sequence. Emit `thinking_end` after the last thinking part before a non-thinking part.
- **OpenAI adapter:** Emit `thinking_start`/`thinking_end` around reasoning summary tokens (if exposed by the Responses API).
- **OpenAI-Compatible adapter:** Same as OpenAI if reasoning tokens are present; no-op otherwise.

The `thinking_delta` event continues to carry the incremental text. The new events are lifecycle markers only (no payload beyond the type).
...
export interface TimeoutConfig {
  /** Total timeout for the entire operation (multi-step generate+tool loop) */
  total_ms?: number;
  /** Per-step timeout for individual LLM calls */
  per_step_ms?: number;
...
`GenerateRequest` gains `timeout?: number | TimeoutConfig`. When a number, it maps to `request_ms` for backward compatibility. Each adapter uses `connect_ms` for the initial connection (via `AbortSignal.timeout()`) and `request_ms` for the full request. `stream_read_ms` is enforced by the SSE parser with a per-chunk deadline reset.

---

## Implementation

### Phase 0: Fix Sprint 022/023 Test Failures (~25%)
...
### Phase 1: Named Retry Preset Policies (~30%)
...
- [ ] Add `retry_policy` as a recognized node attribute in `src/garden/types.ts` (`GardenNode`)
- [ ] Add `default_retry_policy` as a recognized graph attribute in `src/garden/types.ts` (`GardenGraph`)
...
- [ ] Integration test: fixture garden with `retry_policy="patient"` node that retries with longer delays

### Phase 2: Streaming Reasoning Event Lifecycle (~20%)
...
### Phase 3: Error Taxonomy & Structured Timeouts (~15%)

**Files:** `src/llm/errors.ts`, `src/llm/types.ts`, `src/llm/streaming.ts`, `src/llm/retry.ts`, `src/llm/adapters/anthropic.ts`, `src/llm/adapters/openai.ts`, `src/llm/adapters/gemini.ts`, `src/llm/adapters/openai-compatible.ts`, `test/llm/errors.test.ts` (create or extend), `test/llm/client.test.ts`

**Tasks:**

**Error subtypes (GAP-3):**
- [ ] Add `QuotaExceededError` class extending `LLMError` with `retryable = false`
- [ ] Add `StreamError` class extending `LLMError` with `retryable = false` and optional `partial_content`
...
- [ ] All adapters: connection drop mid-stream → `StreamError` with partial content
...
- [ ] Add `resolveTimeout(timeout?: number | TimeoutConfig): ResolvedTimeouts` helper
...
- [ ] Unit tests: resolution logic, backward compat, per-chunk stream timeout
...
| `src/llm/streaming.ts` | Modify | Add `thinking_start` and `thinking_end` event types |
...
| `src/llm/retry.ts` | Modify | Verify skip logic for new error types |
...
| `src/runtime/garden-draft-service.ts` | Modify | Deterministic mock alignment |
...
**Phase 1: Named Retry Presets (GAP-4)**
...
- [ ] Gemini adapter: lifecycle events wrap thinking part sequences
...
**Phase 3: Error Taxonomy & Timeouts (GAP-3, GAP-8)**
- [ ] `QuotaExceededError` has `retryable = false`
- [ ] `StreamError` has `retryable = false` and carries optional `partial_content`
...
- [ ] `stream_read_ms` enforced by SSE parser per-chunk
...
| `pipeline_failed` emission change affects other tests | Medium | Medium | The event should have been emitted all along. Run the full suite after the fix. Any test that was incorrectly passing without `pipeline_failed` needs updating. |
| Retry preset values don't match spec intent | Low | Medium | Values derived from the spec's descriptions (e.g., "patient" = longer delays). Document exact values. Presets are data — trivial to adjust. |
| Reasoning lifecycle events break existing stream consumers | Medium | Medium | New events are additive. Consumers that don't handle `thinking_start`/`thinking_end` simply ignore them. `thinking_delta` is unchanged. |
...
| TimeoutConfig adds complexity to adapter fetch calls | Low | Medium | `resolveTimeout()` centralizes the logic. Adapters call it once and get resolved values. Bare number backward compat means existing code doesn't change. |
| `stream_read_ms` false positives on slow models | Medium | Medium | Default 30s is generous. Anthropic extended thinking can take longer between chunks — the timer resets on each chunk, so thinking is fine as long as tokens arrive. |
...
This sprint adds zero new dependencies. Retry presets are a data table. Streaming lifecycle events extend an existing type union. Error subtypes extend an existing class hierarchy. TimeoutConfig is a new interface consumed by existing adapter code. All changes are localized and additive.

The only prerequisite is that the Sprint 023 codebase compiles and the 964 passing tests remain stable. The 3 failing tests are addressed in Phase 0.
