...
4. **Everything else can wait.** StopReason renaming (GAP-U2), model catalog updates (GAP-U3), truncation tweaks (GAP-C5), ExecutionEnvironment abstraction (GAP-C8), retry defaults (GAP-U11), Response convenience fields (GAP-U5) — these are all real gaps, but none of them produce wrong results at runtime. A sprint that fixes 5 real bugs and ships green tests is worth more than a sprint that attempts 12 fixes and leaves 4 tests red again.
...
- ExecutionEnvironment grep/glob (GAP-C8) — no one swaps environments today
- Retry defaults (GAP-U11) — current values work
- Response convenience fields (GAP-U5) — callers access content directly
- stream() tool loop (GAP-U6) — design question, not a bug
- All low-priority gaps — they're low priority for a reason
- New CLI commands, Hive UI features, or product-facing work
- Medium-priority gaps that are spec-conformance issues rather than correctness bugs
...
This overlaps perfectly with GAP-A3 — the same code path that needs to emit `PipelineFailedEvent` is the same code path that needs to implement the failure routing chain. Both are a restructure of `engine.ts`'s post-failure logic.
...
### Engine Retry & Failure Routing (GAP-A1, GAP-A3)
...
**Change:** The retry check becomes `status === 'retry' || (status === 'failure' && retriesRemaining > 0)`. After retry exhaustion on any node, a new `resolveFailureTarget()` function implements the spec's routing chain:

1. Outgoing edges matching `condition="outcome=failure"` or `condition="outcome=fail"` → select via edge selector
2. Node `retry_target` attribute → jump to that node
3. Node `fallback_retry_target` attribute → jump
4. Graph-level `retry_target` / `fallback_retry_target` → jump
5. None found → emit `PipelineFailedEvent` → terminate pipeline
...
**Current state:** `session.ts:793` returns `{ status: 'failure' }` on loop detection, killing the session.
...
Three independent adapter changes:

- **OpenAI (`openai.ts`):** The `input_image` content type in the Responses API accepts both `{ type: "input_image", image_url: url }` and `{ type: "input_image", source: { type: "base64", media_type, data } }`. Map both URL and base64 image ContentParts to these formats.
- **Anthropic (`anthropic.ts`):** Existing code handles base64 via `source.type = "base64"`. Add URL support via `source.type = "url", url: imageUrl`. Anthropic's Messages API accepts both.
- **Gemini (`gemini.ts`):** Use `inlineData: { mimeType, data }` for base64 images. Use URL-based images via `fileData: { mimeType, fileUri }` if the provider supports it, otherwise download and inline. Add file-path detection: if the image source looks like a local path, read the file and base64-encode it.

### Gemini Tool Call IDs (GAP-U8)

**Current state:** `gemini.ts:268` uses the function name as the tool call ID. When a response contains two calls to `grep`, both get ID `grep`, and the second result overwrites the first.

**Change:** Generate synthetic unique IDs using an incrementing counter per response: `call_0`, `call_1`, `call_2`, etc. Maintain an `idToName` map so tool results route back to the correct function name. This matches the approach the spec recommends.
...
This phase gets 40% of the sprint budget, not 25%. Previous sprints underbudgeted debugging time and it showed. The diagnosis-first approach means: instrument the failing code path, reproduce the failure with added logging, identify the exact root cause, then write the minimal fix.
...
- [ ] Add a comment at each fix site explaining the root cause (spec: "diagnosis-first, not bump-timeout")

### Phase 2: Engine Retry & Failure Routing (~25%)

**Files:** `src/engine/engine.ts`, `test/engine/retry.test.ts`, `test/integration/conditions.test.ts`, `test/fixtures/retry-failure-routing.dot`

**Tasks:**

- [ ] Modify the retry decision in `engine.ts`: `shouldRetry = (status === 'retry' || status === 'failure') && retriesRemaining > 0` (GAP-A1)
- [ ] Extract `resolveFailureTarget(node, graph, context, edgeSelector)` as a standalone function
- [ ] Implement the 5-step failure routing chain in `resolveFailureTarget()`:
...
- [ ] Replace the exit handler's goal-gate routing with a call to `resolveFailureTarget()`
...
**Files:** `src/agent-loop/session.ts`, `test/agent-loop/loop-detection.test.ts`
...
**Files:** `src/llm/adapters/openai.ts`, `src/llm/adapters/anthropic.ts`, `src/llm/adapters/gemini.ts`, `test/llm/adapters/openai.test.ts`, `test/llm/adapters/anthropic.test.ts`, `test/llm/adapters/gemini.test.ts`
...
- [ ] **Gemini:** Add image ContentPart handling:
  ...
  - URL images → `fileData: { mimeType, fileUri }` (Gemini native format)
...
| File | Action | Purpose |
|------|--------|---------|
| `src/engine/engine.ts` | Modify | PipelineFailedEvent emission; retry on `failure`; failure routing chain |
...
| `test/engine/retry.test.ts` | Modify | Failure-status retry + 5-step routing chain tests |
...
- [ ] After retry exhaustion, engine follows 5-step chain: fail-condition edge → node retry_target → node fallback_retry_target → graph-level targets → `PipelineFailedEvent` + terminate (GAP-A3 closed)
...
### Meta
- [ ] All 1046+ previously passing tests still pass (zero regressions)
...
|------|------------|--------|------------|
| Test failures have deeper root causes than diagnosed | Medium | High | Phase 1 gets 40% of sprint budget, not 25%. Diagnosis-first: instrument, reproduce, root-cause, fix. If root cause analysis proves wrong, the extra budget absorbs investigation. No other work starts until tests pass. |
