...
The compliance report has exactly three HIGH-severity gaps. All three live in the coding agent loop:
...
The correct move is **not** to tackle all three at once. C5 (subagents) is a complex orchestration feature that depends on C1/C3 being stable — you cannot safely manage child sessions if the parent session lacks lifecycle state, steering, or clean follow-up semantics. Rushing subagents on top of a stateless session produces opaque background workers that are harder to debug and impossible to steer.
...
C5 (subagents) becomes the obvious Sprint 012, building on a session that actually has state, steering, and provider-specific tool exposure.
...
IDLE ──submit()──> PROCESSING
                      │
                      ├──natural completion, queue empty──> AWAITING_INPUT
                      │                                        │
                      │                                        ├──followUp()──> PROCESSING
                      │                                        ├──close()────> CLOSED
                      │                                        │
                      ├──abort()/fatal──────────────────────> CLOSED
                      │
                      └──steer()──> [message queued, delivered before next LLM call]
```
...
Today `processInput()` creates a throwaway `messages` array per call. This sprint moves conversation history onto the `AgentSession` instance:
...
`followUp()` appends a new user message to the existing conversation. The model sees the full prior context — tool results, its own reasoning, file contents it read — and can build on it instead of re-discovering everything.
...
| Anthropic | Fold into the request's `system` array, preserving order |
| Gemini | Fold into `systemInstruction` content |

This is ~50 lines of adapter code, but it gives `steer()` correct semantics instead of faking it as user text (which confuses models about who is speaking).
...
This is a feature, not a compromise. OpenAI/Codex is better with patches. Anthropic/Gemini are already wired around exact-match edits. One editing primitive forced across all providers is the wrong abstraction.
...
**Strict, not fuzzy.** If context lines don't match, the tool call fails and returns a descriptive error. The model retries with corrected context. Fuzzy matching hides bugs and produces wrong edits.
...
Git snapshot uses bounded-timeout commands (`git` with 2s timeout). If `git` is unavailable or the workspace isn't a repo, the block is silently omitted.
...
- [ ] In the core loop, drain `pendingSteers` as `developer`-role messages before each `client.stream()` call
...
- [ ] Tests: state transitions through all legal paths, steer delivery timing, follow-up reuses conversation, illegal actions rejected, single-input regression (existing behavior unchanged)
...
- [ ] Anthropic adapter: fold `developer` messages into the `system` array, preserving their position relative to other system content
...
- [ ] Create `buildGitSnapshot(workspaceRoot): Promise<string | null>`:
  ...
  - Each git command bounded to 2s timeout
...
- [ ] Tests: developer role reaches each adapter correctly, system prompt contains environment/git blocks inside a git repo, non-git workspace produces no error
...
- [ ] Create `applyParsedPatch(ops: PatchOperation[], env: ExecutionEnvironment): Promise<PatchResult>`:
  ...
  - For Add: verify target doesn't exist (unless overwrite flag)
...
| `src/agent-loop/session.ts` | Modify | State machine, queues, persistent conversation, `submit`/`followUp`/`steer` |
...
| `test/agent-loop/session-control.test.ts` | Create | State machine, steering, follow-up tests |
...
| `test/llm/adapters/anthropic.test.ts` | Modify | Developer role folding |
...
- [ ] `AgentSession` exposes `submit()`, `followUp()`, `steer()`, `getState()`, `close()`
...
- [ ] Anthropic adapter folds `developer` into the system block preserving order
...
| `developer` role folding is lossy on Anthropic/Gemini | Medium | Medium | Fold deterministically in one place per adapter, preserving message order. Adapter-level tests assert exact request payload shape. |
| Git snapshot gathering slows session startup | Low | Medium | Each git command has a 2s timeout. Gather only summary data (counts, not full diffs). Skip entirely on failure — session starts regardless. |
...
**2 HIGH gaps closed. 7 MEDIUM gaps closed. 3 LOW gaps closed. 12 total.**
